...
# Compare helper: is $head fully contained in $base? (ahead_by == 0)
# Returns "merged" / "not-merged" / "missing" (base/head gone).
is_contained_in() {
  local base="$1" head="$2" resp ahead
  resp="$(rest_soft "/repos/$OWNER/$REPO/compare/${base}...${head}")"
  ahead="$(jq -r '.ahead_by // empty' <<<"$resp")"
  if [[ -z "$ahead" ]]; then echo "missing"; return; fi
  [[ "$ahead" == "0" ]] && echo "merged" || echo "not-merged"
}
  ...
  while IFS=$'\t' read -r name date age oid; do
    ...
    for rb in ${RELEASE_BRANCHES[@]+"${RELEASE_BRANCHES[@]}"}; do
      ...
      if [[ "$(is_contained_in "$rb" "$name")" == "merged" ]]; then
        merged_into="$rb"; break
      fi
    done
    # 2) fall back to default branch
    if [[ -z "$merged_into" ]]; then
      if [[ "$(is_contained_in "$DEFAULT_BRANCH" "$name")" == "merged" ]]; then
