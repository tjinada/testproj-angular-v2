#!/usr/bin/env bash
# find-traces-by-url-precise.sh
# Reads URL paths (with optional {placeholder} segments) from a text file and
# queries Dynatrace for traces whose url.path matches the full shape of each
# input pattern. Uses matchesRegex for per-segment precision instead of the
# prefix-contains approach in find-traces-by-url.sh.
#
# Each {anything} placeholder is treated as a single path segment ([^/]+).
# Regex is anchored at start only (^...) so trailing slashes / query strings
# in url.path still match.
#
# Usage:
#   ./find-traces-by-url-precise.sh <input-file> [output-file]
#
...
# Strip a leading HTTP verb + space (GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS).
strip_verb() {
  local s="$1"
  case "$s" in
...
# Build a regex pattern that matches the URL's full shape:
#   - {anything} -> [^/]+   (exactly one path segment)
#   - regex specials in literal segments are escaped
#   - anchored at start only (^...) so trailing chars in url.path still match
# Examples:
#   "/cdb/customer/{OCIFID}/preference/deliveryMethod"
#     -> "^/cdb/customer/[^/]+/preference/deliveryMethod"
#   "/cdb/credit-card/creditcardinfo/eligibility/validation"
#     -> "^/cdb/credit-card/creditcardinfo/eligibility/validation"
build_regex() {
  printf '%s' "$1" | awk '
    BEGIN { ORS=""; specials = ".*+?^$()[]|\\/" }
...
# Build the DQL query for a given regex pattern.
# Uses matchesRegex(url.path, "...") for per-segment precision.
build_dql() {
  local regex="$1"
  printf 'fetch spans, from:-24h, scanLimitGBytes:500 | filter matchesRegex(url.path, "%s") | fieldsAdd _kindRank = if(span.kind == "server", 0, else: 1) | sort _kindRank asc | summarize { startTime = takeMin(start_time) }, by: { trace.id } | sort startTime desc | limit 1' "$regex"
}
...
while IFS= read -r raw_line || [ -n "$raw_line" ]; do
  ...
  # 1. Strip optional HTTP verb prefix.
  # 2. Build a regex pattern from {placeholder} segments.
  # The original $url is kept for display in the CSV.
  stripped=$(strip_verb "$url")
  if [ -z "$stripped" ]; then
    echo "[$index/$total] $url ... SKIPPED (empty after stripping verb)" >&2
    printf '%s,%s,,,\n' "$(csv_escape "$url")" "$(csv_escape "invalid input")" >> "$OUTPUT_FILE"
    continue
  fi
  regex_pattern=$(build_regex "$stripped")

  dql=$(build_dql "$regex_pattern")
